...
# tylko centrowanie: scale(DF$wiek, center = TRUE, scale = FALSE) <- ODCHYLENIE OD ŚREDNIEJ
# tylko skalowanie:  scale(DF$wiek, center = FALSE, scale = TRUE)
# normalizacja
(DF$wiekNorm <- scale(DF$wiek))

DF$wiekNorma2 <- mean(DF$wiek)
DF$kalkulacja <- DF$wiek - DF$wiekNorma2
...
#zmiana "," na "."
tmp <- gsub(pattern = ",",replacement = ".", df$liczby)
df$liczby <- as.numeric(tmp)
...
(sum(auta2012$Przebieg.w.km, na.rm = T))
