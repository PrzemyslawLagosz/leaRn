...
auta_box <- auta2012wybrane[auta2012wybrane$Cena < 10000, ]
...
# cut dzieli kolumne Cena.w.PLN na 100 czesci
# table sumuje produkt cut i pokazuje ile cen wpada do danego predziału np. (1,1000] 23. tzn ze w przedzialae od 1zl do 1000zl jest 23 samochod
#wchich.max wskazuje ten w ktorym jest najwiecej samochodow


which.max(table(cut(auta_box$Cena.w.PLN, 100)))
...
# sapply(wektor, funkcja) -> dla kazdego wystapienie sprawdza klase
# sprawdza czy ta klasa to wektor
# which pozwazal zobaczy indeks kolumny gdzie TRUE. 
# names -> zwraca nazwy kolumn bez jej ideksy
...
auta2012$przedzialy_Rok.Produkcji <- cut(auta2012$Rok.produkcji, c(1900,1990,1995,2000,2005,2010,2012), right = FALSE)
table(auta2012$przedzialy_Rok.Produkcji)

auta2012$przedzialy_Przebieg.w.km <- cut(auta2012$Przebieg.w.km, c(0,1000,10000,100000, 1000000, 10000000))
table(auta2012$przedzialy_Przebieg.w.km)


#Wyznacz tabel? liczebno?ci dla tych dw?ch nowych zmiennych.

tabela_podukcja_przebieg <- xtabs(~przedzialy_Przebieg.w.km + przedzialy_Rok.Produkcji, auta2012)
...
prop.table(table(pojemnosc_i_naped))*100
...
####
zdanie <- c("W Szczebrzeszynie chrz?szcz brzmi w trzcinie", "Z?b zupa z?bowa d?b zupa d?bowa")
