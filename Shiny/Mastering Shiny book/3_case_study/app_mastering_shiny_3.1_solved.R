...
server <- function(input, output, session) {
  ...
  output$age_sex <- renderPlot({
    if (input$y == "count") {
      summary() %>%
        ...
        scale_color_manual(values = c("#47fed3", "#ff00e6"))
    } else {
      summary() %>%
  ...
  # Print status od story to the console
  observeEvent(input$next_story, { message(story()) })
  observeEvent(input$prev_story, { message(story()) })
}
